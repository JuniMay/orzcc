...
impl Global2Local {
    pub fn analyze_usage(&mut self, ctx: &Context) {
        ...
        let insts_in_main = func_main.iter(ctx).flat_map(|block| block.iter(ctx));
    ...
    pub fn change_global_into_local(
        ...
        let insts_get_global = insts_user.clone(); // 此处insts一定都在当前函数中，且使用的global_slot一定是当前的全局槽。
...
impl GlobalPassMut for Global2Local {
    ...
    fn run(&mut self, ctx: &mut Context) -> PassResult<(Self::Output, bool)> {
        ...
        let func_main = ctx.lookup_func(&Symbol::from("main")).unwrap();
        let globals: HashSet<GlobalSlot> = ctx
            .global_slots()
            .iter()
